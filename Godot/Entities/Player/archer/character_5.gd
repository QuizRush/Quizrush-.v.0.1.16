...
var combo = 0
var damage_ = 25  # Set initial damage value
var bulletDirection = Vector2(1, 0)
var arrow = "arrow_2"
var can_attack = true  # Declare can_attack
...
func det_dir(dir):
	if dir != 0:
		...
		animated_sprite.flip_h = dir == -1  # Local flip for the current client
		rpc("sync_flip", animated_sprite.flip_h)  # Sync flip with other clients
...
func attack():
	if Input.is_action_just_pressed("attack") and !anyMovement and can_attack:
		...
		if combo != 3 and $AttackTimer.is_stopped():
			$AttackTimer.start()
			await wait_for_animation("attack_1", 0.7)  # Duration of attack 1
			shoot()
			combo += 1
		elif combo == 3 and $AttackTimer.is_stopped():
			$AttackTimer.start()
