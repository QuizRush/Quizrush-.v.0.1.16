...
func attack():
	if Input.is_action_just_pressed("attack") and !anyMovement and can_attack:
		...
		if combo != 4 and $AttackTimer.is_stopped():
			...
			$AttackTimer.start()
			await wait_for_animation("attack_2", 0.8)  
			shoot()
			combo = 0
			$Node2D.reset_mana()
...
func shoot():
	rpc("rpc_spawn_arrow", maker_2d.global_position, bulletDirection, damage_)
	spawn_arrow(maker_2d.global_position, bulletDirection, damage_)

@rpc
func rpc_spawn_arrow(position: Vector2, direction: Vector2, damage_amount: int):
	spawn_arrow(position, direction, damage_amount)

func spawn_arrow(position: Vector2, direction: Vector2, damage_amount: int):
	var Bullet_node = BULLET.instantiate()
	Bullet_node.set_direction(direction, combo, self, damage_amount)
